...
def choose_file1():
    ...
    if file_path1:
        ...
        messagebox.showinfo("File Selezionato", f"File selezionato: {file_path1}")
...
def choose_file2():
    ...
    if file_path2:
        # Mostra il percorso del file selezionato (puoi gestirlo come preferisci)
        messagebox.showinfo("File Selezionato", f"File selezionato: {file_path2}")
        ...
        with socket.create_connection(server_address) as sock:
            with context.wrap_socket(sock, server_hostname='localhost') as tls_sock:
                ...
                if response == 'Hello from TLS server!':
                    ...
                    certificato1 = file_path1
                    with open(certificato1, "rb") as cert_file1:
                        cert_data1 = cert_file1.read()

                    # Invia i dati al server
                    tls_sock.sendall(cert_data1)
                    ...
                    if response == 'Hello from TLS server!':
                        ...
                        messagebox.showinfo("Schnorr ZKP", "Ti verrà chiesto di caricare la tua chiave privata della prima credenziale, non verrà assolutamente inviata ad alcun server, servirà per eseguire il protocollo ZKP in locale!")
                        ...
                        if response == 'Hello from TLS server!':
                            ...
                            certificato2 = file_path2
                            with open(certificato2, "rb") as cert_file2:
                                cert_data2 = cert_file2.read()

                            # Invia i dati al server
                            tls_sock.sendall(cert_data2)
                            ...
                            if response == 'Hello from TLS server!':
                                ...
                                messagebox.showinfo("Schnorr ZKP", "Ti verrà chiesto di caricare la tua chiave privata della seconda credenziale, non verrà assolutamente inviata ad alcun server, servirà per eseguire il protocollo ZKP in locale!")
...
def main():
    ...
    canvas.create_text(
        ...
        118.0,
        anchor="nw",
        text="Per avere il RdC devi essere maggiorenne ed avere un ISEE < 7.000€ !!!",
        fill="#000000",
